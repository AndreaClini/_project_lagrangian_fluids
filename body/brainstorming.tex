...
\begin{align}
    ...
    &= \sqrt{-g}\,\, \epsilon_{\lambda\mu\nu\rho} \,\,\epsilon_{ijk}\,\, \partial^\mu \phi^i \partial^\nu \phi^j \partial^\rho \phi^k.
\end{align}
Using the Levi-Civita \emph{tensor} $\sqrt{-g}\epsilon_{\alpha\beta\gamma\delta}$ (just the volume form) and the $4D$ tensorial identity
\begin{align}
...
\begin{align}
    T_{\mu\nu}=-\frac{2}{\sqrt{-g}}\frac{\delta S_{\mathrm{fluid}}}{\delta g^{\mu\nu}}={\color{red} \bigl(Y (\partial_Y\!F) - 2 B (\partial_B\!F)\bigr)\,u_\mu u_\nu
           + \bigl(F + Y (\partial_Y\!F) - 2 B (\partial_B\!F)\bigr)\,g_{\mu\nu}}\,.
\end{align}
That is \todotag{Just a guess it is this, finish il conto below}
\begin{align}
     \rho = Y (\partial_Y\!F) - F, \qquad
    p = F+Y (\partial_Y\!F) - 2 B (\partial_B\!F).
\end{align}
%
\begin{proof}
