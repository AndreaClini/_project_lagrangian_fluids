...
Therefore, assuming only the second principle, the condition for posivity reads
\begin{align}
    c_s^2-c_a^2 \geq 0 \quad \Leftrightarrow \quad \frac{1}{(\rho+P)}\bigg\{\underbrace{-T^2\big(\partial_T^2A\big)}_{\geq0}\Big[\big(\partial_V^2A\big)-\tfrac{(\rho+P)}{V}\Big]+\underbrace{\Big[(\rho+P)+T\partial_{TV}^2A\Big]^2}_{\geq 0} \bigg\}\geq 0.
\end{align}
This could equivalently be expressed in terms of the specific free-energy $\mathfrak{a}=A/V$ as
\begin{align}
    c_s^2-c_a^2 \geq 0 \quad \Leftrightarrow \quad \cdots
\end{align}
We might be able to explicitate this condition further using Maxwell relations.\todotag{do it}
In non-phantom regime $\rho+P\geq 0$ the condition simplifies to
\begin{align}
...
%
Now we now translate everything into a condition on $F(b,Y)$.
...
\end{align}
Finally, the condition on the function $F(b,Y)$ for positivity of $c_s^2 - c_a^2\geq 0$ reads \todotag{Check exp for F}
\begin{align}
    ...
    -\big(Y^2b\partial_Y^2\partial_bF - Y^2\partial_Y^2F\big)
    \cdot
    \big(b\partial_bF + b^2\partial_b^2F+b^3\partial_b^3F\big)
    }
    {\partial_YF-b\partial_bF} \geq 0.
\end{align}
}
\bigskip
