...
\end{align}
That is, matching to the perfect fluid expression, we get the expressions from \cite{} \todotag{Does not match flat case... check it back!}
\begin{align}
...
\begin{align}
    ...
    \bigl(Y (\partial_Y\!F) - 2 B (\partial_B\!F)\bigr)\,u_\mu u_\nu
           + \bigl(F - 2 B (\partial_B\!F)\bigr)\,g_{\mu\nu}.
\end{align}
Matching to the perfect fluid expression $T_{\mu\nu}=(\rho+p)u_\mu u_\nu + p g_{\mu\nu}$ we find the expression in \cite{} (ballesteros)\footnote{Note that Ballesteros et al. use b=$\sqrt{B}$ and $F(b,Y)$, so that $\partial_b F = 2 \sqrt{B} \partial_B F$.}
\begin{equation}
    \rho = Y (\partial_Y\!F) - F, \qquad
    p = F - 2 B (\partial_B\!F).
\end{equation}
