...
\begin{equation}
S_2=\int d^4xd^4x\sqrt{-g}\left(\frac{p+\rho}{2}\right)\left[\dot\pi^2-\mathcal{C}_s^2(\nabla\pi)^2+2\mathcal{M}\dot{\pi}_0 (\nabla\!\!\cdot\!\pi)+\mathcal{A} \dot{\pi}_0^2\right]\ , 
\end{equation}
the sound speed is now 
\begin{equation}
\mathcal{C}_s^2= \frac{-b^2F_{bb}-2yF_y-y^2F_{yy}+2byF_{by}}{yF_y-b F_b}\,.
\end{equation}
...
\end{equation}
which shows that the contribution from the entropy mode is strictly positive. Now $c_s^2$ is actually an independent function of $c_a^2$. So that requiring the absence of gradient instabilities does not forbid accelerated expansion which is only constrained by the absence of ghosts. As a consequnce the non-barotropic fluid EFT can span the whole space of 
\begin{equation}
...
\begin{align}
    ...
    (\delta u^j)^\prime+ \mathcal{H}(\delta u^j)-3\mathcal{H}\frac{\bar{P}^\prime}{\bar{\rho}^\prime}(\delta u^j)+\frac{\partial_j\delta P}{(\bar{\rho}+\bar{P})}+\partial_j\Phi{\color{red}+\partial_\ell \Pi^{\ell j}}=0\,.
\end{align}
where we write $u^\mu=\frac{1}{a}(1-\Phi, \delta u^j)$. \todotag{Check convention with Diego}
Introducing the velocity potential $\delta u^j=\partial_j v$ and passing to Fourier space the latter reads
\begin{align}
    \delta\rho^\prime= -3\mathcal{H}(\delta\rho+\delta P) + (\bar{\rho}+\bar{P})\big(3\psi^\prime + k^2 v\big)\,\\
    v^\prime+ \mathcal{H}v-3\mathcal{H}\frac{\bar{P}^\prime}{\bar{\rho}^\prime}v+\frac{\delta P}{(\bar{\rho}+\bar{P})}+\Phi{\color{red}+\tfrac{2}{3}\Pi}=0\,.
\end{align}
and which we can combine in the second order differential equation for the energy density fluctuations\todoAC{Check convention Diego before}
\begin{equation}
 ...   
\end{equation}
Since the entropy is conserved along the flow $\dot{\delta y}=0$\questionAC{Mean $\dot{\delta \sigma}=0$? } we can write the equation in comoving gauge $(\delta p=0)$ and eliminate the non-propagating entropy fluctuation to get an expression for the sound speed similar to the one in 2.34. 
...
\end{equation}
Mi aggiungi il matching per bene qui? pressione e la sua variazione fino alle equazione per $\delta\rho$
A me sembra che $\Gamma$ sia funzione del potenziale ma l'equazione di stato e' non e' solo funzione del potenziale...\answerAC{Dettagli per 5-ess. in appendix, accorcio e scrivo qui}
\vspace{8mm}

Poi generalizza a k-essence e k essence che e' essenzialmente k essence + quintessence.\questionAC{Lo faccio}
...
\section{Adding dark matter and LSS phenomenology}

\section{Introducing bulk viscosity}
...
%--------------------------------------------
\subsection{Adiabatic sound speed}
%------------------------------------------------
...
We also show $c_b^2\equiv c_s^2$ always, which show that absence of ghosts, thermodynamic stability and gradient instabilities are closely related.
Indeed, using the above expressions 
\begin{align}
\begin{aligned}
    c_s^2&=\mathcal{C}^2+\frac{\mathcal{M}^2}{\mathcal{A}}\\
        &= \frac{-b^2F_{bb}-2yF_y-y^2F_{yy}+2byF_{by}
    ...
    {y^2F_{yy}(yF_y-b F_b)}
    \\[6pt]
    &=\frac{-b^2F_{bb} F_{yy}+(F_y-bF_{yb})^2}{F_{yy}\,(yF_y-bF_b)}\equiv c_b^2.
\end{aligned}
\end{align}


%----------------------------------------------------------------
\subsection{Matching to scalar field}
%----------------------------------------------------------------
...
\end{align}
Therefore the \textbf{comoving entropy for the scalar field} is\todoAC{Formula $F_y/b$ does not account for $V$..}
\begin{align}
...
\end{align}
confirming it is not a function of the potential only.
