...
\end{equation}
where CHECK SIGN!!\answerAC{Sign ok, Diego delete when read}
\begin{equation}
    c_s^2=\mathcal{C}_s^2+\frac{\mathcal{M}^2}{\mathcal{A}}\,, 
\end{equation}
which shows that the contribution from the entropy mode is strictly positive. Now $c_s^2$ is actually an independent function of $c_a^2$. So that requiring the absence of gradient instabilities does not forbid accelerated expansion which is only constrained by absence of ghosts. As a consequnce the non-barotropic fluid EFT can span the whole space of 
\begin{equation}
...
\begin{align}
\delta p &=c_b^2\delta\rho+\Gamma\delta \sigma\ ,
\end{align}
where we defined $c_b^2=\partial p/\partial \rho\vert_\sigma$ and $\Gamma=\partial p/\partial \sigma\vert_\rho$. Notice that since $\dot{s}=0$ in a perfect fluid $c_b^2=c_a^2$. 
