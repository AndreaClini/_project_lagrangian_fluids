...
\end{aligned}
\end{align}
Using the first equation, the second can be equivalently rewritten as an evolution equation for $y$
\begin{align}\label{eq:eolvution_y_background}
    \dot{y}=-3\mathcal{H}\frac{F_y-bF_{by}}{bF_{yy}} \overset{!}{=}\dot{b}\frac{dy}{db}_{|\sigma}\,.
\end{align}
...
Note this is precisely the evolution equation along characteristics of the PDE \eqref{eq:characteristics_eos_pde_solution_w_depending_b} for the equation of state with $w=w(b)$, as we will see below!
...
From the solution \eqref{eq:eos_pde_solution_w_depending_b} of the equation of state with parameter $w=w(b)$ we compute
\begin{align}
    F(y,b)=f\Big(y\, e^{-\int_{b_0}^bd\log b\, w(b)}\Big)\, e^{\int_{b_0}^bd\log b\, \big(1+w(b)\big)} \quad \Rightarrow\quad  \frac{F_y-bF_{by}}{F_{yy}} \equiv w(b)\frac{y}{b}.
\end{align}
Combining this with \eqref{eq:eolvution_y_background} we indeed get back the prescribed evolution of $y$ along characteristics of the PDE \eqref{eq:characteristics_eos_pde_solution_w_depending_b}, which is simply evolution in time since $a\propto b^{-1/3}$,\todotag{check}
\begin{align}
\begin{aligned}
...
\end{aligned}
\end{align}
As argued after \eqref{eq:constant_eos_w_solution_pde} the above solution $F$ of the EoS PDE should be consistent with the two Noether currents, i.e. the continuity equation, and the first Friedmann equation.

We now solve Friedman and continuity equation, consistenly with the possible time variation of $w$, \todotag{Finish}
\begin{align}
