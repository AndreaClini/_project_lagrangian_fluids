...
\end{align}
Plugging this into \eqref{eq:eolvution_y_background} we get
{\small\begin{align}\label{eq:evolution_y_background_equation_final}
\begin{aligned}
    \frac{\dot{y}}{y}=w(b)\frac{\dot{b}}{b}=-3w(a)\mathcal{H}
\end{aligned}
\end{align}}
This is precisely the characteristic ODE \eqref{eq:characteristics_eos_pde_w_depending_b}, and the solution \eqref{eq:characteristics_eos_pde_solution_w_depending_b} is rewritten in terms of $a$ or $b$ respectively as 
...
\end{align}
Expanding $\dot{F}=F_b\dot{b}+F_y\dot{y}$ etc one verifies the above is automtically satified by \eqref{eq:conservation_noether_current_background_eft}.
