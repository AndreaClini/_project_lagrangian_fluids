...
In turn, the proportionality of $y$ versus the scale factor $a$ will be generally different from the one of the Boltzmann temperature as discussed after \eqref{eq:bkg_scaling_y_coordinate}.
...
\end{align}}
where $\Xi(x^0)$ is a background function of time only, which is fixed by demanding conservation of Noether currents and consistency of the background evolution (cf. Section \ref{sec:bkg_evolution_fluid_EFT}). 
...
We first consider first the flat spacetime case for simplicity,\todotag{Do it \& also highlight this combination in the $\Box$ action} but we anticipate the final form is unchanged upon allowing mixing with metric perturbations and considering the actual entropy perturbations $\left(\tfrac12 h^{00}+ \dot{\pi}^0\right)$.
Recalling $\dot{\pi}^0=\tfrac{\mathcal{M}}{\mathcal{A}}\Big[(\partial_j\pi^j)+ E(\mathbf{x})\Big]$ from \eqref{eq:integrate_out_entropy_flat_spacetime} and the expressions for \eqref{eq:adiabatic_cs_nonadiabatic_pressure} $c_a^2$ and $\Gamma$, after some math we find
\begin{align}\label{eq:rho_p_sigma_fluctuations_vs_phonon_entropy_field}
\begin{aligned}
    \delta\rho &= (yF_y-bF_b)(\partial_j\pi^j)+(yF_y-byF_{by})E(\mathbf{x}) = (\bar{\rho}+\bar{p})(\partial_j\pi^j)\,+by\,\delta\sigma \,,\\[2.5pt]
    \delta\sigma &= \tfrac{yF_y-byF_{by}}{by}E(\mathbf{x})=\frac{\partial_yp}{b}E(\mathbf{x})\,,\\[2.5pt]
