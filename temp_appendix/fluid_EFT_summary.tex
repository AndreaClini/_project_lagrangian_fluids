...
On the other hand, we shall see that the background value of $\varphi^0(x^\mu)$ will be naturally imposed by the conservation of the entropy current.\todotag{Fix explanation}
...
\textcolor{red}{Crucially what we call temperature $y=T$ here is not the usual temperature of a thermal distribution, and should be more abstractly identified as `the variable conjugate to the entropy'.}\todotag{Expand}
...
\end{align}
In the barotropic case, case adiabatic sound speed, gradient coefficient and ratio of time variations of pressure to density all coincide.
...
For convenience, we compare our formalism to Wayne Hu's \cite[equation (3)]{Hu:1998kj} we find
\begin{align}
...
Hence no ambiguity in the variables to keep fixed when computing $dp/d\rho$ and \eqref{eq:barotropic_speed_of_sound} holds, which now further reduces to
\begin{align}
...
We now expand to quadratic order around unperturbed\footnote{Beware this choice of fluid coordinates does not use any gauge freedom of the spacetime manifold!} fluid coordinates $\bar{\varphi}^\alpha\equiv x^\alpha$ and FLRW spacetime.\todotag{No, $\varphi^0$ is kept free}
...
{\small\begin{align}\label{eq:perturbed_fluid_coordinates}
    \varphi^0=\gamma\Big(\Xi(\tau)+\pi^0\Big), \quad \varphi^i=\lambda\Big(x^i+\pi^i\Big)\,, \quad [\varphi^\alpha]=[x^\alpha]=[\pi^\alpha]=E^{-1},\quad [\gamma]=[\lambda]=E^0,
\end{align}}
\textcolor{red}{where $\Xi(\tau)$ is a background function of time only, which will be fixed by demanding conservation of the Noether currents and consistency of the background evolution.}\todotag{Polish}
...
\begin{aligned}
    ...
    \\[3pt]
    &y= \bar{y}(1+\delta y),\quad \bar{y}= \frac{\gamma\,\,\Xi(\tau)}{a},\quad \delta y^{(1)} = \tfrac12 h^{00}+ \dot{\pi}^0,\\[3pt]
...
Finally we compute\todoAC{FINISH!!}
\begin{align}
    \delta\sigma = F_{yy}\delta y + \frac{bF_{yb}-F_y}{b^2} \delta b = ...
\end{align}
Finally let us compare variations of $\delta p$ and $\delta \rho$.
Recalling that $\dot{\pi}^0=\tfrac{\mathcal{M}}{\mathcal{A}}(\partial_j\pi^j)+ E(\vec{x})$ and recalling the expression for $c_a^2$ and $\Gamma$ and doing the math we miracoulously find
\begin{align}
    \delta p = c_a^2 \delta\rho + \Gamma \delta\sigma .
\end{align}
where we must identify $\delta \sigma = by\, E(x^i)$.
The entropy fluctuations are thus the leftover of the out integration of $\pi^0$.
%------------------------------------------------------------------------
