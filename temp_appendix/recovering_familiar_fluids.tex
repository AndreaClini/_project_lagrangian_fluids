...
In this section we want to get some intuition for the form of the function $F(y,b)$ that describes some familiar fluids, such as cold dark matter and radiation.
Hopefully this will also reveal how the fluid EFT is more general than the usual scalar field description of fluids.
Recall the general expressions \eqref{eq:thermo_rho_p_nonbaro}-\eqref{eq:adiabatic_cs_nonadiabatic_pressure} for the nonbarotropic case and \eqref{eq:thermo_variables_barotropic} for the barotropic case.

%========================================================================
\subsection{Fluids with constant EoS parameter {\color{red}(in progress)}}
%========================================================================
...
%-----------------------------------------------------------------------
%
Demanding Dark Matter be a barotropic fluid and imposing $w=0$ we get
\begin{align}
...
In particular we also find $c_a^2=\tfrac13$ necessarily
\begin{align}
...
This begs the question:  are there \emph{nonbarotropic} relativistic fluids, in the fluid EFT this simply means enforcing $w=\tfrac13$, where however $c_a^2\neq\tfrac13$?\, NO.
...
\end{align}
This is just expected: since $p=w\rho$ with $w$ constant, pressure is a function of energy density only, so the fluid is barotropic and adiabatic by construction.

%-------------------------------------------------------------------------
\subsubsection{Fluids with arbitrary constant EoS parameter {\color{red}(in progress)}}
%-------------------------------------------------------------------------
...
\end{align}
The first order PDE is solved by characteristic.
Choosing variables $\gamma:=\log uy$ and $\beta:=\log b$, the characteristics are
{\small
\begin{align}\begin{aligned}
    \dot{\beta}&=1\\
    \dot{\gamma}&=w \quad \Rightarrow\quad y = C b^w\, \\
    \dot{z}&=(1+w)z\,.
\end{aligned}\end{align}}
This confirms that a fluid with constant EoS parameter $w$ is ultimately barotropic, i.e. it depends only on $b$, since we can take $y=C b^w$.
The most general solution is, for an arbitrary function $f=f(z)$ on one variable,
\begin{align}\label{eq:constant_eos_w_solution_pde}
    F(y,b)=f(y\,b^{-w})\, b^{(1+w)}, 
\end{align}
which reduces to the barotropic case simply taking $y\propto b^w$ so that $f(yb^{-w})\mapsto \mathrm{const}$.

The implications of $w=\mathrm{const}$ on $c_a^2$ and $\Gamma$ defined in \eqref{eq:adiabatic_cs_nonadiabatic_pressure} are obvius, since $p=w\rho$ with $w$ constant implies $p=p(\rho)$.
Indeed, differentiating the relation \eqref{eq:constant_eos_w} with respect to $y$ and $b$ respectively we get
\begin{align}
    y^2F_{yy}=t\frac{1}{w}\big(yF_y-byF_{by}\big), \qquad b^2 F_{bb}=w\big(bF_b-ybF_{yb}\big),
\end{align}
and plugging this into \eqref{eq:adiabatic_cs_nonadiabatic_pressure} we explicitly verify
{\small\begin{align}
\begin{aligned}
...
\end{aligned}
\end{align}}
As expected, a fluid with constant EoS is purely barotropic in practice!


%========================================================================
\subsection{Fluids with time depdendent EoS parameter $w=w(a)$ {\color{red}(in progress)}}
%=======================================================================
%
Despite reducing to the barotropic case, the solution for constant $w$ is nonetheless useful since a simple generalization is to consider a time-varying EoS parameter $w=w(a)$ with the scale factor $a$, but otherwise independent of $b,y$.
The PDE is still formally solved by the above expression
\begin{align}
    F(y,b)=f(y\,b^{-w(a)})\, b^{(1+w(a))}.
\end{align}
We should now impose a consistency relation on $f$ to ensure 1st Friedmann equation and continuity are satisfied \todotag{check if it makes sense}
\begin{align}
    (\dot{a}/a)^2=\frac{1}{3M_{\mathrm{Pl}}^2}\rho=\frac{1}{3M_{\mathrm{Pl}}^2}(-F+yF_y)\\
    \dot{\rho}=-3\mathcal{H}(\rho+p)=-3\mathcal{H}(-bF_b+yF_y)
\end{align}
This will give consistency constraints on $f$ and its derivatives for the equations to hold.
Indeed, for a given form of $f$ we can invert the relations to express $b,y$ in terms of $\rho,p$ and then check if the above equations are satisfied.


%=======================================================================
\subsection{Fluids with arbitrary EoS parameter $w(b,y)$ {\color{red}(in progress)}}
%==========================================================================

More generally we can solve by characteristics the EoS for arbitrary parameters $w=w(b,\gamma)$\todotag{not sure it is needed}
\begin{align}
    \frac{F-F_b}{yF_y-F}=w\approx \mathrm{const}\quad\Rightarrow\quad (1+w)F=w\, yF_y+bF_b.
\end{align}
This is just a linear transport equation.
Choosing variables $\gamma:=\log y$ and $\beta:=\log b$, the characteristics are
{\small
\begin{align}\begin{aligned}
    \dot{\beta}&=1\\
...
Crucially, we cannot say anything more on $c_a^2$ or $\Gamma$for a completely general $w(y,b)$.
...
\paragraph{The relevant case.}
The equation is quite simple when variables split $w=w_1(y)w_2(b)$.
Since the conservation of the Nother current gives $b\propto a^3$ at the backgorund level, a very intteresting case is  $w=w(b)$ since it ultimately corresponds to $w=(a)$ up to perturbations,
\begin{equation}
    (1+w(b))F=w(b)\, yF_y+bF_b.
\end{equation}
...
\begin{aligned}
    \dot{\beta}&=1\\
    \dot{\gamma}&=w(\beta)\\
    \dot{z}&=\big(1+w(\beta)\big)z
\end{aligned}\end{align}
}
The characteristic simply becomes
{\small
\begin{align}\label{eq:characteristics_eos_pde_solution_w_depending_b}
\begin{aligned}
