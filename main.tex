...
It is now straightfroward to compute $\frac{\partial P}{\partial \sigma}_{|\rho}$.
We can proceed as before enforcing 
\begin{equation}
    0=d\rho=yF_{yy}dy+(yF_{yb}-F_b)db\quad \Rightarrow\quad \frac{dy}{db}_{|\rho}=\frac{F_b-yF_{yb}}{yF_{yy}}
\end{equation}
Then again
\begin{align}
    \Gamma:=\frac{dp}{d\sigma}_{\mid\rho}&=\frac{p_b + p_y\, \tfrac{dy}{db}_{\mid\rho}}{\sigma_b + \sigma_y\, \tfrac{dy}{db}_{\mid\rho}}=\frac{-bF_{bb}+(F_y-bF_{by})\tfrac{F_b-yF_{yb}}{yF_{yy}}}{\Big(\tfrac{bF_{by}-F_y}{b^2}\Big)+\tfrac{F_{yy}}{b}\frac{F_b-yF_{yb}}{yF_{yy}}}
    \\
...
\include{matching_scalar_field}

\include{recovering_familiar_fluids}
