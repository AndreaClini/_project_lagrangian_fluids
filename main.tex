...
\end{equation}
the sound speed is now {\color{red} (likely wrong see appendix)}
\begin{equation}
\mathcal{C}_s^2= \frac{-b^2F_{bb}-2yF_y-y^2F_{yy}+2byF_{by}}{yF_y-b F_b}\,.
\end{equation}
The mixing of the entropy mode with the phonons is given by  {\color{red} (likely wrong see appendix)}
\begin{equation}
...
\section{Adding dark matter and LSS phenomenology (next in line)}
...
{\color{red}
The computation below is wrong since both $\mathcal{C}_s^2$ and $\mathcal{M}$ contain wrong extra terms.
Still these terms cancel between original and entropy contribution to $c_s^2$ and we still get $c_b^2=c_s^2$.
Th correct computation in in appendix B.}\todotag{Remove this when polished}
...
\include{quadratic_action_curved_spacetime}
