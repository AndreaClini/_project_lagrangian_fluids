...
In this section we fix notations, conventions, and report relevant formulas in Cosmology so that we do not to waste time looking them up or worrying about conventions, signs etc. \notetag{This sec will be shortened at the end}
...
The SVT decomposition of a general perturbated FLRW metric is written as 
{\small\begin{align} \label{eq:metric_general_form_svt}\begin{aligned}
g_{\mu\nu}dx^\mu& dx^\nu=a^{2}(\tau)\left[-(1+2 A) \,\mathrm{d} \tau^{2}+2 B_{i} \mathrm{~d} x^{i} \mathrm{~d} \tau+\left((1+2C)\delta_{ij}+2E_{ij}\right) \mathrm{d} x^{i} \,\mathrm{~d} x^{j}\right],\\[3pt]
&\underbrace{A,\, C}_{\text {scalar }}, \qquad
B_{i}=\underbrace{\partial_{i} {B}}_{\text {scalar }}+\underbrace{\hat{B}_{i}}_{\text {vector }},\qquad E_{i j}=\underbrace{\partial_{\langle i} \partial_{j\rangle} {E}}_{\text {scalar }}+\underbrace{\partial_{(i} \hat{E}_{j)}}_{\text {vector }}+\underbrace{\tilde{E}_{i j}}_{\text {tensor }},
...
\end{align}}
For reference, in terms of a general perturbation we have
{\small\begin{align}\label{eq:general_perturbations_vs_SVT_decomposition}
\begin{aligned}
    &g_{\mu\nu}=a^2(\eta_{\mu\nu}+h_{\mu\nu}), \quad g^{\mu\nu}=\tfrac{1}{a^2}(\eta^{\mu\nu}-h^{\mu\nu}), \quad h^{\mu\nu}=\eta^{\mu\alpha}\eta^{\nu\beta}h_{\alpha\beta}, \\[3pt]
    &h_{00}=-2A, \quad h_{0i}=B_i, \quad h_{ij}=2C\delta_{ij}+2E_{ij}.
\end{aligned}\end{align}}
...
\begin{aligned}
    u^\mu=\frac{1}{a}\left(1-A,\,-B_i+v_i\right),\quad u_\mu=a\left(-(1+A),\,v_i\right),\quad v^i=\delta^{ij}v_j,\\[2pt]
...
%=================================================
Under a gauge transformation $x^\mu \mapsto x^\mu +\delta x^\mu$ with $\delta x^\mu=(T,\,\partial^i L+\hat{L}^i)$, the metric pertubations transform as
\begin{align}\label{eq:gauge_transformation_metric}
\begin{aligned}
    \begin{array}{ll}
...
\begin{align}\label{eq:gauge_transformation_em_tensor}
\begin{aligned}
    \delta \rho & \mapsto \delta \rho-\dot{\bar{\rho}}\, T, \quad\qquad v_{i}  \mapsto v_{i}+\dot{L}_{i},\\[2pt]
    \delta P & \mapsto \delta P-\dot{\bar{P}}\, T, \,\,\qquad \Sigma_{i j}  \mapsto \Sigma_{i j}.
\end{aligned}
\end{align}
...
Moreover on large scales $\zeta \simeq \mathcal{R}$ since the gauge-invariant Poisson equation \eqref{eq:poisson_gauge_invariant} and the 1st Friedman equation give\footnote{The second equality holds for total species and for separately conserved $\nabla_\mu T^\mu_0=$ species.}
{\small
\begin{align}
\begin{aligned}
...
%
For reference, the Christoffel symbols in this gauge
\begin{align}\label{eq:christoffel_newtonian_gauge}
    ...
    \end{aligned}
\end{align}
The perturbed Einstein equations in this gauge, with (i,j)-component already split int trace and traceless part and $s$ denoting different species, are
\begin{align}
...
\end{align}
A linear combination of the 00 and 0i Einstein equations gives the gauge-invariant Poisson equation
\begin{align}\label{eq:poisson_gauge_invariant}
    \nabla^2\Phi= 4\pi G a^2 \bar{\rho}\Delta\, ,\quad  \text{for}\quad \Delta:=\delta +\frac{\dot{\bar{\rho}}}{\bar{\rho}}(v+B),
\end{align}
...
{\small\begin{subequations}\label{eq:continuity_euler_bkg_perturbed}\begin{align}
    &\dot{\bar{\rho}} +3\mathcal{H}(\bar{\rho}+\bar{p})=0\,, \label{eq:background_continuity}\\[3pt]
    &\dot{\delta\rho}= -3\mathcal{H}(\delta\rho+\delta P) + (\bar{\rho}+\bar{P})\big(3\dot{\phi}-\partial_j v^j \big)\,,\label{eq:perturbed_continuity_drho}\\[3pt]
...
%=======================================================================
\subsection{Other gauges \& ADM decomposition}
%====================================================================
...
In particular recall in this gauge there is some residual gauge freedom that can be fixed by demanding the peculiar velocity of a \emph{cold} ($w\simeq 0$) species vanish.\questiontag{Finish this if needed eventually cf. \cite{Weinberg_Cosmology_2008}}
...
\end{aligned}
\end{align}}
where $\gamma_{ij}$ contains the transverse-traceless vector and tensor modes, and the matrix exponential is in the Taylor sense.
