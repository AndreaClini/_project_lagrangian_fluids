...
In partcular, at some fixed initial time $x^0=\tau_{i}$, we can choose $\varphi^i(x)=x^i$ for $i=1,2,3$ and $\varphi^0(x)=\bar{\varphi}^0(x^0)$ for some function $\bar{\varphi}^0$ of time only, whose form will be enforced by consistency with the other backrgound equations cf.\eqref{eq:evolution_y_background_from_definition}, and then allow for perturbations $\pi^\alpha(x^\mu)$ around them as in \eqref{eq:perturbed_fluid_coordinates}.
...
After normalizing the 4-velocity we get, where $b$ is defined below,
...
\end{align}
We identify 2 adimensional quantities $b$ and $y$ as
...
\end{align}
The symmetries of the system imply conservation of a number density current (from volume-preserving diffeo invariance in the $\varphi^i$) and an entropy current\footnote{\label{fn:thermodynamic_interpretation}See next subsection for the jutification of the thermodynamic interpretation.} (from shift symmetry in $\varphi^0$)
\begin{align}\label{eq:noether_currents_nonbaro}
    \nabla_\mu J^\mu=0\quad\text{for}\qquad J_n^\mu = b\, u^\mu\,,\qquad J_s^\mu = F_y \,u^\mu\,.
\end{align}
The first conservation is a `Bianchi identity' i.e. a purely geometrical constraint, while the latter is a genuine Noether current conservation that holds only on-shell.
...
\begin{align}\label{eq:barotropic_speed_of_sound}
    \frac{dp}{d\rho}=\frac{\tfrac{dp}{d\hat{x}}}{\tfrac{d\rho}{d\hat{x}}}=\frac{\tfrac{dp}{dt}\cancel{\tfrac{d\hat{x}}{dt}}}{\tfrac{d\rho}{dt}\cancel{\tfrac{d\hat{x}}{dt}}}= \frac{\dot{p}}{\dot{\rho}}=w-\frac{\dot{w}\bar{\rho}}{\dot{\bar{\rho}}}\quad \forall  \text{ directions } \hat{x}, \quad \text{and}\qquad c_a^2=c_s^2=c_g^2.
\end{align}
...
Crucially, if the relative variation of the EoS parameter is small compared to the Hubble rate
\begin{align}
    \frac{\dot{w}/w}{\dot{\bar{\rho}}/\bar{\rho}}\simeq \frac{\tfrac{\dot{w}}{w}}{-3\mathcal{H}(1+w)}\ll 1 \quad \Rightarrow \quad c_s^2\simeq w,
\end{align}
...
As discussed above for the general barotropic case, there is now only one independent thermodynamic variable, which can be any of $b$, $\rho$ etc. 
...
In fact this notion of entropy density has even wrong dimension $[F_y]=E^4$ and should be rescaled in order to identify it with the familiar one.
...
%=============================================================================()
\subsection{Background evolution in the fluid EFT\textcolor{red}{(IN PROGRESS)}}\label{sec:bkg_evolution_fluid_EFT}
%==============================================================================
%
We consider first the conservation of the number density and entropy currents.
...
Finally, from the very definition of $y=u^\mu\partial_\mu\varphi^0$ and $\bar{u}^\mu\overset{\text{bkg}}{=}a^{-1}\delta^\mu_0$ we have the background evolution
\begin{align}\label{eq:evolution_y_background_from_definition}
    \bar{y}=\partial_\mu\bar{\varphi}^0 \bar{u}^\mu = \tfrac{1}{a}\tfrac{d\bar{\varphi}^0}{d\tau}.
\end{align}
We will then choose $\bar{\varphi}^0(\tau)$ to be consistent with the other background equations.
...
The equation of state is then enforced by solving the associated pde
\begin{align}\label{eq:equation_of_state_eos_pde}
    \frac{F-F_b}{yF_y-F}=w(b,y)\quad\Rightarrow\quad (1+w)F=w\, yF_y+bF_b.
\end{align}
This is a simple transport equation that is solved by characteristics for arbitrary $w=w(y,b)$ and will constrain the form of $F$.
The solution is temporarily reported in Section \ref{sec:fluids_arbitrary_eos} and will be moved here.\todoAC{Polish and do it}

Having enforced the desired EoS parameter $w(b,y)$ by writing $F$ as \eqref{eq:eos_pde_solution_w_depending_b}, and having demanded the conservation of Noether currents in \eqref{eq:conservation_noether_current_background_eft} and thus the continuity equation, we finally impose the last background equation (1st Friedmann).
\begin{align}
    &\mathcal{H}^2 = \frac{8\pi G}{3} a^2 \rho = =\frac{8\pi G}{3} a^2 (-F+yF_y)\,.
\end{align}
In the following we first discuss the case $w=w(b)$ and then the general case $w=w(y,b)$ which might be needed for the matching to quintessence.


%=============================================================================()
\subsubsection{Background for $w=w(b)$\textcolor{red}{(IN PROGRESS)}}\label{sec:bkg_evolution_eft_wb}
%==============================================================================
%
Crucially, when $w=w(b)$, equation \eqref{eq:eolvution_y_background} is equivalent to the characteristic ODE \eqref{eq:characteristics_eos_pde_w_depending_b}-\eqref{eq:characteristics_eos_pde_solution_w_depending_b} of the equation of state.
Evolution in time thus coincides with evolution along characteristics \eqref{eq:eos_pde_w_depending_b} in this case.
...
Indeed, from the general solution \eqref{eq:eos_pde_solution_w_depending_b} of the EoS with  $w=w(b)$ we compute
\begin{align}
...
\end{aligned}
\end{align}}
We can now specify the background value $\varphi^0(x^\mu)=\bar{\varphi}^0(x^0)+\pi^0(x^\mu)$ for the time component of the fluid internal coordinates \eqref{eq:perturbed_fluid_coordinates}.
...
\begin{aligned}
    &y=u^\mu\partial_\mu\varphi^0 \overset{\text{bkg}}{=}\tfrac{1}{a}\mathcal{H}\frac{d\bar{\varphi}^0}{d\log\!a}\propto\, \exp\Big(\!\!-\!\!3\!\!\int^a_{a_0}\!\!\!\!\!d\!\log(a)\,\, w(a)\Big)\\[2.5pt]
    &\quad\Rightarrow\quad \bar{\varphi}^0(a)=\int^a_{a_0}\!\!\!d\log\!a\, \frac{a}{\mathcal{H}}e^{-3\int^a_{a_0}\!d\!\log(a)\,\, w(a)}\,,
\end{aligned}\end{align}
up to a nonphysical constant shift in time $\bar{\varphi}^0\mapsto \bar{\varphi}^0 +C$.
...
Having enforced the desired EoS parameter $w(b)$ by writing $F$ as \eqref{eq:eos_pde_solution_w_depending_b}, and having demanded the conservation of Noether currents in \eqref{eq:conservation_noether_current_background_eft} and thus the continuity equation, we finally impose the last background equation (1st Friedmann).
...
\end{aligned}
\end{align}
The energy density is expressed as a function of the scale factor $a$, depending on the form of $w(a)$, and the initial condition $\rho_0=y_0f^\prime(y_0)-f(y_0)$.
...
\end{align}
For any given functional form $w(a)$, this fixes the time evolution of the scale factor $a(\tau)$.
The case of familiar species is obtained for constant $w=-1,0,1/3$.


%=============================================================================()
\subsubsection{Background for $w=w(b,y)$\textcolor{red}{(IN PROGRESS)}}\label{sec:bkg_evolution_eft_wby}
%==============================================================================
%
The above analysis already shows that it won't be enough to take $w=w(b)$ to match quintessence.
Let's do some naive power counting.
The first current conservation \eqref{eq:conservation_noether_current_background_eft_number} fix the scaling $b\propto a^{-3}$.
The second \eqref{eq:conservation_noether_current_background_eft_entropy} is recast in the form \eqref{eq:eolvution_y_background} to fix the scaling of $y$, but we still have complete freedom in the choice of $\bar{\varphi}^0$ to fulfil this last condition.
The continuity equation is auotomatically satisfied.
We showed above that an EoS with $w=w(b)=w(a)$ constrains the form of $F$ and in turn does fix $\bar{\varphi}^0$.
The function $F$ is now fully specified up two constant initial value and the free function $w(a)$.
The 1st Friedmann equation fixes the scaling $\tau \mapsto a(\tau)$ for arbitrary $w=w(a)$.
The only freedom left is thus in the choice of $a\mapsto w(a)$.
If we now want to match a given EoS parameter $w=w(a)$ e.g. the quintessence one, we completely fix the form of $w(a)$ and there is no freedom left.
It would seem that we have no freedom left to match an arbitrary $c_a^2$, e.g. the quintessence one $c_a^2=1$, since this value should automatically follows from the above constrains. 

If we instead consider $w=w(y,b)$ we might be able to preserve the independence of $w$ and $c_a^2$...\todotag{Finish}
...
\end{aligned}
\end{align}}
The fluid variables are written as perturbations around background values\footnote{We could also insert dimensionless rescaling factors $\varphi^0\mapsto \gamma \varphi^0$ and $\varphi^j\mapsto \lambda \varphi^j$ as in \cite{Dubovsky:2005xd} to reabsorb measure units, but avoid it to keep the notation cleaner.} as discussed after equation \eqref{eq:internal_coord_fluid}, 
{\small\begin{align}\label{eq:perturbed_fluid_coordinates}
    \varphi^0=\bar{\varphi}^0(x^0)+\pi^0(x^\mu), \quad \varphi^i=x^i+\pi^i(x^\mu)\,, \quad [\varphi^\alpha]=[x^\alpha]=[\pi^\alpha]=E^{-1}.
\end{align}}
...
\begin{aligned}
    &\pi^0 \mapsto \pi^0 - \dot{\bar{\varphi}}^0\,T,\qquad \pi^i \mapsto \pi^i - \partial^i L - \hat{L}^i.
\end{aligned}\end{align}}
...
\begin{aligned}
    &\dot{\pi}^0/\dot{\bar{\varphi}}^0+\tfrac12 h^{00} \mapsto \dot{\pi}^0/\dot{\bar{\varphi}}^0+\tfrac12 h^{00} +\big(\mathcal{H}-\ddot{\bar{\varphi}}^0/\dot{\bar{\varphi}}^0\big) T, \\
...
\begin{aligned}
    &b= \bar{b}(1+\delta b),\quad \bar{b}= a^{-3},\quad
    \delta b^{(1)} = (\partial_j \pi^j)-\tfrac12 h^{ii},\\[3pt]
    ...
    \\[3pt]
    &y= \bar{y}(1+\delta y),\quad \bar{y}= a^{-1}\dot{\bar{\varphi}}^0,\quad \delta y^{(1)} =\dot{\pi}^0/\dot{\bar{\varphi}}^0  +\tfrac12 h^{00},\\[3pt]
    &\delta y^{(2)} = \tfrac12 h^{00}\dot{\pi}^0/\dot{\bar{\varphi}}^0+ h^{0 i} \dot{\pi}^i  + \tfrac12 (\dot{\pi}^i)^2 -\dot{\pi}^j\,\partial_j\pi^0/\dot{\bar{\varphi}}^0\,.
\end{aligned}\end{align}}
...
\end{align}
Matching the velocity expansion \eqref{eq:expansion_u_4_velocity} in the fluid EFT to the general expression \eqref{eq:velocity_decomposition} in pertrubed FLRW spacetime $au^\mu =(1\tfrac12 h^{00},h^{0\ell}+ v^\ell)$ and recalling \eqref{eq:general_perturbations_vs_SVT_decomposition}, we identify the peculiar velocity and the velocity potential/divergence in terms of phonons as
{\small
\begin{align}\label{eq:peculiar_velocity_fluid_vs_phonons}
    h^{0\ell}+v^\ell = \frac{1}{2}\epsilon_{i j k}\,\epsilon^{\ell \alpha j k}\,\partial_\alpha\pi^i = -\dot{\pi}^\ell\quad \Rightarrow \quad \begin{array}{c} v^\ell=-(\dot{\pi}^\ell+h^{0\ell}),\quad v = -\dot{\pi}_L+B\,,\\[2.5pt]
        \theta=\partial_\ell v^\ell=-\partial_\ell\big(\dot{\pi}^\ell+h^{0\ell}\big)= -\nabla^2\big(\dot{\pi}_L-B\big)\,.\end{array}
\end{align}}
...
\begin{aligned}
    ...
    \\
    &= (y^2F_{yy})\left(\dot{\pi}^0/\dot{\bar{\varphi}}^0  +\tfrac12 h^{00}\right) + (-bF_b+byF_{by})\left((\partial_j \pi^j)-\tfrac12 h^{ii} \right) + O(2)\,,
    \\[3pt]
    \delta p &= (yF_y-byF_{by})\delta y - (b^2 F_{bb})\delta b
    \\
    &= (yF_y-byF_{by})\left(\dot{\pi}^0/\dot{\bar{\varphi}}^0  +\tfrac12 h^{00}\right) - (b^2 F_{bb})\left((\partial_j \pi^j)-\tfrac12 h^{ii}\right)+ O(2)\,,
    \\[3pt]
    \delta\sigma &= \tfrac{yF_{yy}}{b}\delta y + \tfrac{bF_{yb}-F_y}{b} \delta b
    \\
    &= \tfrac{y^2F_{yy}}{yb}\left(\dot{\pi}^0/\dot{\bar{\varphi}}^0  +\tfrac12 h^{00}\right) +\tfrac{ybF_{yb}-yF_y}{yb}\left((\partial_j \pi^j-\tfrac12 h^{ii} )\right)+ O(2)\,.
\end{aligned}
\end{align}
We now express pressure fluctuations in terms of energy density and entropy ones.
Using the flat \eqref{eq:integrate_out_entropy_flat_spacetime} or curved \eqref{eq:integrate_out_entropy_curved_spactime} spacetime expression to integrate out the combination $\left(\dot{\pi}^0/\dot{\bar{\varphi}}^0  +\tfrac12 h^{00}\right)$ of entropy field and metric perturbations, and recalling the expressions \eqref{eq:adiabatic_cs_nonadiabatic_pressure} for $c_a^2$ and $\Gamma$, after some math we find
\begin{align}\label{eq:rho_p_sigma_fluctuations_vs_phonon_entropy_field}
\begin{aligned}
    \delta\rho &= (yF_y-bF_b)(\partial_j\pi^j-\tfrac12 h^{ii})+\tfrac{\dot{\bar{\varphi}}^0}{a^4}E(\mathbf{x}) = (\bar{\rho}+\bar{p})(\partial_j\pi^j-\tfrac12h^{ii})\,+by\,\delta\sigma \,,\\[2.5pt]
    \delta\sigma &= \frac{\dot{\bar{\varphi}}^0}{a^4by}E(\mathbf{x})\,,\\[2.5pt]
    \delta p &= c_a^2\, \delta\rho + \Gamma \, \delta\sigma\,.
\end{aligned}
\end{align}
As expected, phonons $\partial_j\pi^j$ only source density fluctuations, while the entropy field $\pi^0$ sources both density and entropy fluctuations. 
...
It is worth jumping directly to Section B5 where the equations are easily derived from the hamiltonian formalism.\answerAC{Polish above \& refer to B5}

%==========================================================================
%\subsection{Quadratic action for phonons}
%==========================================================================
%
\input{appendice_andrea/quadratic_action.tex}

%==========================================================================
%\subsection{Phonon Hamiltonian \& conservation equations}
%==========================================================================
%
\input{appendice_andrea/hamiltonian_phonons.tex}
